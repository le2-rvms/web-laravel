...
## 数据链路
1) 首屏数据：`GET /api-admin/gps-data/latest` 从 `timescaledb.gps_position_last` 拉取（可传 `device_ids[]` 过滤；`COMPANY_ID` 存在时按 `tenant_id` 限制）。  
...
```
3) 广播：推送 `GpsPositionUpdated` 事件到私有通道 `gps.company.{company_id}.device.{terminal_id}` 与 `gps.company.{company_id}.device.all`，`coord_sys=GCJ02`，`source=pgsql_listen`。  
4) 推送模式：事件使用 `ShouldBroadcastNow` 同步广播（当前不走队列；如需异步再改为队列广播）。

## 通道与订阅
- 通道名：`gps.company.{company_id}.device.{terminal_id}`（单设备）、`gps.company.{company_id}.device.all`（全设备），均为 PrivateChannel。
  - Echo 订阅：`Echo.private('gps.company.{company_id}.device.all')` / `Echo.private('gps.company.{company_id}.device.{terminal_id}')`。
  - 原生 Reverb/Pusher：频道名为 `private-gps.company.{company_id}.device.all` / `private-gps.company.{company_id}.device.{terminal_id}`（自动加 `private-` 前缀）。
...
- 通道授权（在 `routes/channels.php` 中定义，由 `BroadcastServiceProvider` 加载）：
  ...
  - 设备通道：校验 `company_id` 与当前应用租户一致，且终端号属于当前租户设备。
  - 全量通道：校验 `company_id` 与当前应用租户一致，且具备权限后才可订阅。
  - 失败即拒绝订阅，并记录原因（脱敏）。

## 前端接入流程
- 全图监控：进入页面先调用初始位置 API（`GET /api-admin/gps-data/latest`，`device_ids[]` 为空=全量），渲染 marker；然后订阅 `gps.company.{company_id}.device.all` 接收增量。
