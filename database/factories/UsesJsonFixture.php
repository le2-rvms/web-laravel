...
trait UsesJsonFixture
{
    ...
    protected function randomAdminID($size = 2, $query_func = null): array|int
    {
        ...
        if (null === $datas) {
            $query = Staff::query()->where('user_type', '!=', AdmUserType::TEMP);
            if ($query_func) {
                $query_func($query);
            }
    ...
    protected function randomVehicleServiceAdminID($size = 2): array|int
    {
        ...
        if (null === $datas) {
            $query = Staff::query()->where('user_type', '!=', AdmUserType::TEMP)->whereLike('name', '%'.ImportAdminAndRoles::role_vehicle_service.'%');
            $datas = $query->pluck('id')->toArray();
        }
